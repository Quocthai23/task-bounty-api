...
export class TasksController {
  ...
  findAllByProject(@Param('projectId') projectId: string, @Query() query: PaginationQueryDto) {
    return this.tasksService.findAllByProject(projectId, query.page || 1, query.limit || 10);
  }
  ...
  getPublicTasks(@Query() query: PaginationQueryDto) {
    return this.tasksService.getPublicTasks(query.page || 1, query.limit || 10);
  }
  ...
  getJoinedTasks(@Request() req: any, @Query() query: PaginationQueryDto) {
    return this.tasksService.getJoinedTasks(req.user.sub, query.page || 1, query.limit || 10);
  }
  ...
  @ApiResponse({ status: 401, description: 'Unauthorized' })
  @ApiResponse({ status: 403, description: 'Forbidden - Insufficient permissions to mark DONE' })
  ...
  addComment(@Param('id') id: string, @Request() req: any, @Body() dto: CreateCommentDto) {
    return this.tasksService.addComment(id, req.user.sub, dto);
  }
